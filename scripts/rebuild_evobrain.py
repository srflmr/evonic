#!/usr/bin/env python3
"""
rebuild_evobrain.py — backfill existing long-term memories into a structured,
graph-enabled evobrain brain.
...
Usage:
    python3 scripts/rebuild_evobrain.py --all [--with-graph]
    python3 scripts/rebuild_evobrain.py --agent siwa [--with-graph]
"""
...
from backend.agent_runtime import evobrain_writer as W
...
def _stats(agent_id: str) -> dict:
    ...
    if not os.path.exists(os.path.join(brain_dir, ".evobrain.db")):
        return {}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Backfill memories into evobrain.")
    ...
    if not is_available():
        print("evobrain binary not available — aborting.")
        return 1
