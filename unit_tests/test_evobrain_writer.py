"""Tests for evobrain_writer.py — structured markdown writer.

These tests write to a temporary brain dir (no evobrain binary needed): the
writer is pure Python. `_ensure_brain` is patched to True so writes proceed
without a real `.evobrain.db`.
"""
...
class TestMarkDirty:
    def test_schedules_debounced_sync(self, monkeypatch):
        ...
        monkeypatch.setattr(W, "_evobrain_sync", lambda agent_id: calls.append(agent_id))
        W.mark_dirty("a1")
        import time
        time.sleep(0.2)
        assert calls == ["a1"]
