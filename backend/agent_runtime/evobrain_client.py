...
# Operational tracing for evobrain internals, shared by all evobrain modules.
# Set EVOBRAIN_VERBOSE=1 to emit these traces at INFO level (so they appear in
# normal logs); otherwise they go to DEBUG.
vlogger = logging.getLogger("evobrain")
_EVOBRAIN_VERBOSE = os.environ.get("EVOBRAIN_VERBOSE", "").strip().lower() in (
    "1", "true", "yes", "on")
if _EVOBRAIN_VERBOSE and vlogger.level == logging.NOTSET:
    vlogger.setLevel(logging.DEBUG)


def vlog(msg, *args):
    """Emit an evobrain operational trace (INFO when EVOBRAIN_VERBOSE, else DEBUG)."""
    vlogger.log(logging.INFO if _EVOBRAIN_VERBOSE else logging.DEBUG, msg, *args)


def _summarize(parsed) -> str:
    """Compact one-line description of a parsed evobrain JSON result."""
...
def get_engine() -> str:
    """Return the active primary memory engine ('evobrain' or 'fts5').

    Evobrain is the default. It transparently downgrades to FTS5 when the
    ...
    if engine not in ("evobrain", "fts5"):
        engine = "evobrain"
    if engine == "evobrain" and not is_available():
        return "fts5"
    return engine
...
def init_brain(agent_id: str) -> bool:
    ...
    if not is_available():
        return False
    db_path = os.path.join(brain_dir, ".evobrain.db")
