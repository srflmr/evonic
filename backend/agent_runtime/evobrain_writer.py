"""
evobrain_writer.py — write structured markdown into an agent's evobrain.

Evobrain treats disk as the source of truth: pages are markdown files, the
...
Typed edges recognised by evobrain (from a page body):
...
from backend.agent_runtime.evobrain_client import (
    _get_brain_dir, init_brain, sync as _evobrain_sync, vlog,
)

logger = logging.getLogger(__name__)

# Debounce window (seconds) for coalescing a burst of writes into one sync.
_SYNC_DEBOUNCE_SECONDS = float(os.environ.get("EVOBRAIN_SYNC_DEBOUNCE", "2"))

# Edge types evobrain understands (others fall back to a plain mention).
...
def sync_now(agent_id: str) -> bool:
    ...
    try:
        return _evobrain_sync(agent_id)
    except Exception:
        return False
